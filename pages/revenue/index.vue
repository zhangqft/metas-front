		...
		<view class="content">
			...
			<view class="card">
				...
				<view class="title flex" style="width: 100%;">
					...
					<view style="margin-left: 20rpx; font-weight: bold;font-size: 32rpx;">
						METAS
					</view>
			...
			<view class="pledgePopup">
				...
				<view class="" v-if="!succeed">
					...
					<view class="form">
						<view class="row">
							...
							<view class="input flex">
								<input type="number" placeholder="0" placeholder-class="placeholderClass"
									v-model="query.num" @input="queryChange" />
								<view class="text">
									METAS
								</view>
						...
						<view :class="['submit', { submit_active: releaseFee }]" @click="release"
				...
				<view class="" v-if="!succeed">
					...
					<view class="form">
						<view class="row" style="margin-bottom: 32rpx;">
							...
							<view class="input flex">
								<input type="number" placeholder="0" placeholder-class="placeholderClass"
									v-model="withdrawQuery.num" @input="withdrawQueryChange" />
								<view class="text">
									METAS
								</view>
						...
						<view :class="['submit', { submit_active: requireUsdtValue }]" @click="applyWithdraw"
							v-if="schedule == 1">
							{{ requireUsdtValue ? $t('ratify_nums', { nums: requireUsdtValue }) : $t('withdraw_btn')
							}}
						</view>
						<view :class="['submit', { submit_active: requireUsdtValue }]" v-else>
							{{ $t('deal') }} <u-loading mode="circle" color="278ffe"></u-loading>
						</view>
						<view class="desc">
							<view class="">
							...
							<view class="">
								1.{{ $t('withdraw_describe_1') }}
							</view>
							<view class="">
								2.{{ $t('withdraw_describe_2') }}
							</view>
						</view>
					</view>
				</view>
				...
				<view class="" v-if="!succeed">
					<view class="form">
						...
						<view class="form_item bold flex">
							...
							<view class="">
								{{ withdrawInfo.value }} METAS
							</view>
						</view>
						<view :class="['submit submit_active']" @click="confirmWithdraw(withdrawInfo)"
							v-if="schedule == 1">
							{{ $t('ratify_send') }}
						</view>
...
export default {
	...
	methods: {
		...
		async release() {
			...
			const allowanceValue = await this.$etherCall.contactFunctionCall(erc20Abi, "allowance", [uni
				.getStorageSync("walletAccount"), this.$config.release_contract
			], this.$config.usdt_contract)
			if (ethers.formatEther(allowanceValue.result) < this.query.num) {
				await this.$etherCall.contactFunctionSend(erc20Abi, "approve", [this.$config.release_contract,
				ethers.MaxUint256
				], this.$config.usdt_contract)
			}
			...
			if (this.releaseFee > 0) {
				...
				const usdtValue = await this.$etherCall.contactFunctionCall(releaseAbi, "get_buy", [ethers.parseEther(this.releaseFee.toString())], this.$config.release_contract)
				if (Number(this.usdtBalance) < 0.1 + Number(ethers.formatEther(usdtValue.result))) {
					this.errorInfo = "USDT Balance not enough"
					this.succeed = 2
					return;
				}
			}
			...
			this.$api.awardRelease({
				...
				if (res.code == 0) {
					...
					this.$etherCall.contactFunctionSend(releaseAbi, "release",
						[releaseValue, res.data.days, releaseId, res.data.utc, res.data.v, res.data.r,
							res.data.s
						],
		...
		async computeWithdrawUsdt() {
			if (this.withdrawQuery.num) {
				const usdtValue = await this.$etherCall.contactFunctionCall(withdraw0Abi, "get_buy", [ethers.parseEther(this.withdrawQuery.num.toString())], this.$config.withdraw0_contract)
		...
		async applyWithdraw() {
			this.schedule = 2
			const allowanceValue = await this.$etherCall.contactFunctionCall(erc20Abi, "allowance", [uni.getStorageSync("walletAccount"), this.$config.withdraw0_contract], this.$config.usdt_contract)
			...
			this.$api.applyWithdraw({
				...
				if (res.code == 0) {
					...
					this.$etherCall.contactFunctionSend(withdraw0Abi, "withdraw",
						...
						this.$config.withdraw0_contract).then((res) => {
							if (res.error) {
								this.succeed = 2
							} else {
								this.$api.applyWithdrawConfirm({
									"applyId": applyId,
									"txHash": res.transactionHash,
								}).then(res => {
									if (res.code == 0) {
				...
				if (res.code == 0) {
					...
					if (res.data.awardType == 3) {
						this.$etherCall.contactFunctionSend(withdraw1Abi, "withdraw_complete",
							...
							this.$config.withdraw1_contract).then((res) => {
								if (res.error) {
									this.succeed = 2
								} else {
									this.succeed = 1
					...
					if (res.data.awardType == 5) {
						...
						this.$etherCall.contactFunctionSend(nodeAbi, "income",
							[this.$config.metas_contract, withdrawValue, applyId, res.data.utc, res.data.v,
							res.data.r, res.data.s
							],
							this.$config.node_contract).then((res) => {
								if (res.error) {
									this.succeed = 2
								} else {
									this.succeed = 1
